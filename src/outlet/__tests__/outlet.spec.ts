
// @ts-nocheck

import { Outlet } from '../index'

/*
  TODO: Find a solution for TypeError: Cannot convert a BigInt value to a number at Math.pow (<anonymous>)
  Thrown by import {Authenticator} from '@tokenscript/attestation' due to @tokenscript/attestation/src/libs/Point.ts
*/

describe('Outlet spec', () => {
	test('placeholder test', () => {
		expect(true).toBe(true)
	})
	
	test('placeholder test', () => {
		const outlet = new Outlet({
			"collectionID": "devcon", 
			"onChain": false, "title": 
		"Devcon", "image": "https://raw.githubusercontent.com/TokenScript/token-negotiator/main/mock-images/devcon.svg",
			"tokenOrigin": "http://localhost:3002/",
			"attestationOrigin": "https://test.attestation.id/",
			"base64senderPublicKeys": {
				"6": ""
			},
			"base64attestorPubKey": "",
			"ticketIssuesUrlWebsitePrivateKey": ""
		});

		const issuerTokens = [
			{  ticketIdNumber: 1 },
			{  ticketIdNumber: 2 },
			{  ticketIdNumber: 1 },
			{  ticketIdNumber: 3 },
			{  ticketIdNumber: 3 },
			{  ticketIdNumber: 5 },
			{  ticketIdNumber: 5 },
			{  ticketIdNumber: 6 },
			{  ticketIdNumber: 6 }
		];

		const noDuplicateTokens = outlet.removeDuplicatesByKey(issuerTokens, 'ticketIdNumber');

		expect(JSON.stringify(noDuplicateTokens)).toBe("[{\"ticketIdNumber\":1},{\"ticketIdNumber\":2},{\"ticketIdNumber\":3},{\"ticketIdNumber\":5},{\"ticketIdNumber\":6}]");

	})

})
