...
/* describe('core Spec', () => {
	...
	test('expect to decode missing params', () => {
		...
		expect(decodeTokens(rawTokens, tokenParser, unsignedTokenDataName)).toEqual([])
	})
	...
	test('expect store tokens inside cookies', () => {
		...
		expect(global.localStorage.getItem('testing')).toBe(
			JSON.stringify([
				{ class: 'abc', type: 'gold' },
				{ class: 'def', type: 'silver' },
				{ class: 'ghi', type: 'bronze' },
