...
it("get correct nft tokens query string evm", async () => {
  ...
  expect(query).toEqual("https://api.token-discovery.tokenscript.org/get-owner-tokens?smartContract=0x0d0167a823c6619d430b1a96ad85b888bcf97c37&chain=eth&owner=0x1&blockchain=evm");
});
...
it("get solana tokens query to return undefined when some key params are not given", async () => {
  ...
  expect(query).toEqual("https://api.token-discovery.tokenscript.org/get-owner-tokens?chain=mainnet&blockchain=evm&owner=0x1&updateAuthority=CCCUzWanUNegjGby11DjujDvPaNN68jd9Rimwk2MZzqZ");
});
