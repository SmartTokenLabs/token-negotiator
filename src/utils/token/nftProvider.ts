...
const getSolanaNftTokensUrl = (issuer: any, owner: string, ipfsBaseUrl: string) => {
	const { chain, contract, symbol, updateAuthority, tokenProgram } = issuer;
	const blockchain = issuer?.blockchain ?? "ethereum";
	let query = `${baseURL}/get-owner-tokens?blockchain=${blockchain}&chain=${chain}&owner=${owner}&collectionAddress=${contract}&symbol=${symbol}&updateAuthority=${updateAuthority}&tokenProgram=${tokenProgram}`;
	if (ipfsBaseUrl) query += `&ipfsBaseUrl=${ipfsBaseUrl}`;
	return query;
}
