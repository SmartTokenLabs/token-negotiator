...
export class AttributeTypeAndValue {
	@AsnProp({ type: AsnPropTypes.ObjectIdentifier }) type!: string;

	// @AsnProp({ type: AsnPropTypes.Any })//ANY -- DEFINED BY AttributeType
	// value: ArrayBuffer;
	// hardcode as UTF8String
	@AsnProp({ type: AsnPropTypes.Utf8String }) value!: string;
}
...
export class Name {
	@AsnProp({ type: RDNSequence }) rdnSequence?: RDNSequence;
	@AsnProp({ type: AsnPropTypes.Null }) null?: any;
}
