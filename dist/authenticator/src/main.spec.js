...
let useAttestRes, sessionKey, session2Key, userKey, attestorPubKey, attestorKey, sessionSignature, useAttestationJson, attestationRequestJson, requestAttestAndUsage, useRequestAttestationJson;
...
describe("executeEipFlow", () => {
    ...
    test('executeEipFlow - verify-usage(external json and subtle signature)', async () => {
        let res;
        try {
            res = await Authenticator.verifyUsage(useAttestationJson, attestorPubKey, sessionMessage, WEB_DOMAIN, sessionSignature);
        }
        catch (e) {
            console.error(e);
    ...
    test('executeEipFlow - useAttest', async () => {
        ...
        try {
            useAttestRes = await Authenticator.useAttest(attestationPEM, attestationSecretPEM, attestorPubKey, email, type, WEB_DOMAIN, sessionKey, userKey);
...
describe("executeCombinedEipFlow", () => {
    ...
    test('verify-usage(java-generated request-attest-and-usage)', async () => {
        let res;
        try {
            res = await Authenticator.verifyUsage(useRequestAttestationJson, attestorPubKey, sessionMessage, ATTESTOR_DOMAIN, sessionSignature2);
            if (debugLog) {
    ...
    test('request-attest-and-usage', async () => {
        try {
            requestAttestAndUsage = await Authenticator.requestAttestAndUsage(userKey, "test@test.ts", "mail", ATTESTOR_DOMAIN, attestationSecretPEM, sessionKey);
        }
        catch (e) {
            console.error(e);
    ...
    test('verify-usage(js-generated request-attest-and-usage)', async () => {
        let res;
        try {
            res = await Authenticator.verifyUsage(requestAttestAndUsage, attestorPubKey, sessionMessage, ATTESTOR_DOMAIN, sessionSignature);
        }
        catch (e) {
            console.error(e);
            throw new Error('verifyUsage failed');
